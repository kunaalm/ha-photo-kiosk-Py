...
caption) that the frame page cycles through. The interface is deliberately
tiny so new sources are mechanical additions: implement ``Source``, register
it in ``get_source()``.

The kiosk serves photos from a LOCAL directory (the default and the primary
use case for a homelab kiosk). To show Google Photos, sync them into the
photo directory with an external tool — the kiosk itself touches no cloud
API. See docs/google-photos.md.
"""
